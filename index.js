...
class Database
{
  ...
  static expandArgs(...args)
  {
    ...
    if (data instanceof Array)
    {
      ...
      let i = 0;
      const result = query
        .replace(/\?/g, () => Database.quote(Database.safe(data[i++])));
    ...
    const result = query.replace(re, (a, b) =>
      Database.quote(Database.safe(data[b])));
  ...
  runQueries(queries, returnResult, callback)
  {
  ...
  each(...args)
  {
    ...
    const complete = typeof args.at(-2) === 'function' ?
      args.pop() : undefined;
    const callback = args.pop();
